...
data = pd.read_csv('./神经网络基础与Tensorflow实战/housing.csv', header=None, delim_whitespace=True)
...
# 保存模型（用于后续加载预测，使用 .keras 单文件格式避免 Windows 下目录保存异常）
model.save('boston_model.keras')
# 保存标准化器（预测时需用相同方式预处理输入）
with open('scaler.pkl', 'wb') as f:
    pickle.dump(ss_input, f)
print('模型已保存至 boston_model.keras，标准化器已保存至 scaler.pkl')